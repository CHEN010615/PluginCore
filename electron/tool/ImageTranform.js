...
class ImageTransform {
  ...
  async imgCompress(options) {
    ...
    // 构建输出文件的完整路径
    const outputDir = path.dirname(`${outputPath}/${relativePath}`);
    // 确保输出目录存在
    fs.mkdirSync(outputDir, { recursive: true });
    // 构建输出文件的完整路径
    const outputFilePath = `${outputDir}/${path.basename(filePath)}`;
  ...
  async compressToSize(options, compressOptions, recursiveImage = false) {
    ...
    const cpoyPath = `${outputPath.split('/').slice(0, -1).join('/')}/${copyFileName}`;
    ...
    if (recursiveImage && compressOptions.quality > 10) {
      compressOptions.quality -= 5; // 每次压缩降低5%质量
    } else if (recursiveImage) {
      return; // 质量已经很低了，不再继续压缩
    }
